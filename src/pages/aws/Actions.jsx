...
const Actions = ({ params, rowId, setRowId }) => {
  ...
  const createSnapshotCall = async ()=>{
    setIsOpen(false);
    const instanceSnapshotName = `${params.row.name}-${moment().format("MMDDYYYYhmmss")}`
    contextValue.setIsLoading(true);
    const res = await createSnapshot('aws', {provider: "AWS", instanceName: params.row.name, instanceSnapshotName: instanceSnapshotName})
    console.log(res)
  ...
  const startInstanceCall = async ()=>{
    setIsOpen(false);
    contextValue.setIsLoading(true);
    const res = await startInstance('aws', {provider: "AWS", instanceName: params.row.name})
    console.log(res)
    contextValue.setIsLoading(false);
    if (res.status) {
  ...
  const restartInstanceCall = async ()=>{
    setIsOpen(false);
    contextValue.setIsLoading(true);
    const res = await restartInstance('aws', {provider: "AWS", instanceName: params.row.name})
    console.log(res)
    contextValue.setIsLoading(false);
    if (res.status) {
  ...
  const stopInstanceCall = async ()=>{
    setIsOpen(false);
    contextValue.setIsLoading(true);
    const res = await stopInstance('aws', {provider: "AWS", instanceName: params.row.name})
    console.log(res)
    contextValue.setIsLoading(false);
    if (res.status) {
