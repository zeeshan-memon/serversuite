...
const Actions = ({ params, rowId, setRowId }) => {
  ...
  const createSnapshotCall = async ()=>{
    ...
    const res = await createSnapshot('alicloud', {provider: "Alicloud", instanceId: params.row.instanceId, instanceName: params.row.instanceName})
    console.log(res)
  ...
  const startInstanceCall = async ()=>{
    setIsOpen(false);
    contextValue.setIsLoading(true);
    const res = await startInstance('alicloud', {provider: "Alicloud", instanceId: params.row.instanceId})
    console.log(res)
    contextValue.setIsLoading(false);
    if (res.status) {
  ...
  const restartInstanceCall = async ()=>{
    setIsOpen(false);
    contextValue.setIsLoading(true);
    const res = await restartInstance('alicloud', {provider: "Alicloud", instanceId: params.row.instanceId})
    console.log(res)
    contextValue.setIsLoading(false);
    if (res.status) {
  ...
  const stopInstanceCall = async ()=>{
    setIsOpen(false);
    contextValue.setIsLoading(true);
    const res = await stopInstance('alicloud', {provider: "Alicloud", instanceId: params.row.instanceId})
    console.log(res)
    contextValue.setIsLoading(false);
    if (res.status) {
  ...
  const getSnapshots = ()=>{
    navigate(`/alicloud/snapshots/${params.row.instanceId}`)
  }
  return (
    <MainContaoiner>
      {/* <ListIcon /> */}
